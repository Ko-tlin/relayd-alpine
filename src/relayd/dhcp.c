...
static void parse_dhcp_options(struct relayd_host *host, struct dhcp_header *dhcp, int len) {
	...
	while((uint8_t *) opt < end) {
		...
		switch(opt->code) {
			case DHCP_OPTION_ROUTER:
				...
				break;
			case DHCP_OPTION_ROUTES: DPRINTF(2, "Found a DHCP static routes option, len=%d\n", opt->len); break;
			case DHCP_OPTION_END: opt = (void *) end; continue;
			default: DPRINTF(3, "Skipping unknown DHCP option %02x\n", opt->code); continue;
		}
	}
}
