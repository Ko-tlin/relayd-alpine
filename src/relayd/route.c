...
static void rtnl_rule_request(struct relayd_interface *rif, int flags) {
	struct {
		struct nlmsghdr nl;
		struct rtmsg rt;
...
static struct rtnl_addr *rtnl_add_addr(struct rtnl_addr *addr, int *len, int type, const uint8_t *ipaddr) {
...
static void rtnl_route_request(struct relayd_interface *rif, struct relayd_host *host, struct relayd_route *route, bool add) {
	static struct {
		struct nlmsghdr nl;
		struct rtmsg rt;
	...
	if(route) {
		DPRINTF(2, "%s: add route to " IP_FMT "/%d via " IP_FMT " (%s)\n", ifname, IP_BUF(route->dest), route->mask, IP_BUF(host->ipaddr), host->rif->ifname);
		...
		DPRINTF(2, "%s: add host route to " IP_FMT " (%s)\n", ifname, IP_BUF(host->ipaddr), host->rif->ifname);
...
found_interface:
	...
	for(rta = NDA_RTA(r); RTA_OK(rta, len); rta = RTA_NEXT(rta, len)) {
		switch(rta->rta_type) {
			case NDA_LLADDR: lladdr = RTA_DATA(rta); break;
			case NDA_DST: ipaddr = RTA_DATA(rta); break;
			default: break;
		}
	}
